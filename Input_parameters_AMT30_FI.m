...
#Step 1: `with_ACS122' - run 1 
##UWAY_WAP_SUBDIR = "with_ACS122/"; 
##inidate = inidstr;
##enddate = enddstr; 
##WAPhour = "005"; 
...
% DY157 Instruments Used:
    % BB3-1173
    % CTD
    % ACS-122
strcmp(UWAY_WAP_SUBDIR, "/") == 1 % tjor: this is the `default" config (i.e. without subdirectories inside WAP_extracted)
     dh8_instruments = {"bb3", "ctd", "acs"};
    % Ports must corresponds to same ports as in dh8_instruments
     dh8_ports = {1,2,7}; % AMT30 working setup 
    % Serial numbers are mainly needed for acs and ac9 config files, leave blank for other instruments
##     dh8_serialnumber = {1173, [], 122}; % 20230222 to 20230316
     dh8_serialnumber = {1173, [], 167}; % 20230319 to end


    
##if strcmp (UWAY_WAP_SUBDIR, "with_ACS122/") == 1 % # case with 2 acs instruments - start of cruise
...
PATH_SHIP = [PATH_DATA, "Ship_Data/"]; % note: some cruises store as: [PATH_DATA, "ship_uway/"]
PATH_GPS = [PATH_SHIP,'GPS/'];  % ok
PATH_ATT = [PATH_SHIP,'ATT/'];  % ok
PATH_DEPTH = [PATH_SHIP,'EA600/'];  % ok 
PATH_TS = [PATH_SHIP,'SURFMETV3/']; % ok
PATH_TSG = [PATH_SHIP,'TSG/']; % ok
